...
void	notice(std::vector<std::string> *vec, User & usr, Server & srv)
{
	...
	if (vec->size() < 2)
	{
		...
		return;
	// :ergo.test 461 fg notice :Not enough parameters
	}
	if (srv.find_user(vec->front()) != false)
	{
		notice_user(vec, usr, srv);
	}
	else if (srv.find_channel(vec->front()) != false)
	{
		notice_channel(vec, usr, srv, msg);
	}
	else
	{
		if (vec->front()[0] != '#')
...
bool	notice_user(std::vector<std::string> *vec, User & usr, Server & srv)
{
	...
	if (srv.find_user(vec->front()) != false)
	{
		...
		// msg.erase(0,1);
		send_update(usr, "NOTICE " + nick, msg, i);
...
bool	notice_channel(std::vector<std::string> *vec, User & usr, Server & srv, std::string msg)
{
	...
	while(it != srv.get_end_channel())
	{
		...
		{
			send_general_update(usr, it->second, "NOTICE " + vec->front(), msg, true);
			usr.set_last_activity(time(0));
			return true;
		}
		it++;
